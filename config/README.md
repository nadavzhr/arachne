...
Arachne is configured using YAML files. This directory contains the system-wide settings and the registry of job sources.
...
- **`concurrency`**: Max number of sources to scrape simultaneously.
...
- **`enabled`**: Toggle a source on or off globally.
