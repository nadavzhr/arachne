...
-   **`spiders/`**: Tests for spider-specific normalization logic.
-   **`utils/`**: Unit tests for shared helper functions.

## Strategy

### 1. Mocking External APIs
We do not make real network calls during testing. We use `pytest-mock` to stub the `fetch` methods of spider adapters, returning pre-defined "raw" payloads to test the `normalize` logic.
...
### 3. Snapshot Testing
For spider adapters, we store sample JSON responses from the real APIs and verify that our normalization logic produces the expected `JobPosting` objects.
