...
-   **`logging.py`**: A specialized logging system that supports central logging and per-source log isolation.
-   **`clients/`**: HTTP and Browser client abstractions.
-   **`config/`**: Configuration loading and Pydantic validation for global and source-specific settings.
...
-   **`sources/`**: The extensibility point. Contains adapters for various company job portals.
...
3.  **Dispatch**: For each enabled source, a task is spawned in the `asyncio` event loop.
4.  **Fetch & Normalize**: The source adapter's `fetch()` method retrieves raw data, and its `normalize()` method converts it into `JobPosting` objects.
