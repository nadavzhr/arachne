...
- Maps raw YAML data into `GlobalConfig` and `SpiderConfig` Pydantic models.
...
- Handles the inheritance/override logic where spider-specific settings take precedence over global profile settings.

## Data Models
We use Pydantic models to represent configuration:
- **`GlobalConfig`**: System-level settings.
- **`SpiderConfig`**: Per-spider registry settings (URL, enabled status).
